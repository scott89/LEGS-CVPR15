...
static shared_ptr<Net<float> > net_, dnng, dnnl;
...
static void get_weights(MEX_ARGS) {
 if (nrhs != 1) {
  ...
  const string net = mxArrayToString(prhs[0]);
  if (net == "DNNG") {
    net_ = dnng;
  } else if (net == "DNNL") {
    net_ = dnnl;
  } else {
    mexErrMsgTxt("Unknow network. Network name should be either DNNG or DNNL");
  }
  prhs = prhs + 1;
...
static void init(MEX_ARGS) {
  if (nrhs != 4) {
    ostringstream error_msg;
    error_msg << "Expected 4 arguments, got " << nrhs;
    mex_error(error_msg.str());
  }
  const string net = mxArrayToString(prhs[0]);
...
static void set_input_dim(MEX_ARGS) {
  ...
  net_->set_input_dim(id, num, ch, h, w);
}
